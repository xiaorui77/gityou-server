...
import static com.gityou.server.Utils.ServletUtils.ATTRIBUTE_HANDLER;
...
public class GitSmartHttpTools {
     ...
     *
     * @param req the current HTTP request that may have been made by Git.
     ...
     * @param httpStatus HTTP status code to set if the client is not a Git client.
     * @throws IOException the response cannot be sent.
     */
     ...
     * @param res        current response.
     * @param httpStatus HTTP status code to set if the client is not a Git client.
     * @param textForGit plain text message to display on the user's console. This is
    ...
    public static void sendError(HttpServletRequest req,
        ...
        if (isInfoRefs(req)) {
            ...
            if (httpStatus < 400)
                org.eclipse.jgit.http.server.ServletUtils.consumeRequestBody(req);
     ...
     * @throws IllegalArgumentException the request is not a Git client request. See
    ...
    public static String infoRefsResultType(String svc) {
     ...
     *
     * @param pathOrUri path or URI of the request.
